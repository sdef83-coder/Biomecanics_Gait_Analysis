...
# 2. Fonction GVI en version Boxplot
create_gvi_boxplot <- function(df) {
  ...
  ggplot(df_plot, aes(x = AgeGroup, y = Value, fill = Surface)) +
    ...
    # Labels
    labs(
      title = "Gait Variability Index (GVI) across age groups and surfaces",
...
fig_gvi_box <- create_gvi_boxplot(data)
print(fig_gvi_box)

# Enregistrement haute qualité
ggsave("Figure_GVI_Maturation_Surface.pdf", width = 10, height = 8)
ggsave("Figure_GVI_Maturation_Surface.png", plot = fig_gvi_box, width = 10, height = 7, dpi = 300)