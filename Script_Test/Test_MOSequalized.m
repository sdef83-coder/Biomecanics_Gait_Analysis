...
sujet_id = 'CTL_01';                              % ID du sujet à traiter
...
output_file_csv = sprintf('C:\\Users\\silve\\Desktop\\DOCTORAT\\UNIV MONTREAL\\TRAVAUX-THESE\\Surfaces_Irregulieres\\Datas\\Script\\gaitAnalysisGUI\\result\\MoS\\MoS_%s.csv', sujet_id);
...
EQUALIZE_CYCLES_BETWEEN_SURFACES = true;   
RANDOM_SEED_EQUALIZATION = 42;             % seed repro pour la sélection
TARGET_LEFT_RIGHT_RATIO = 0.5;            % 50% gauche / 50% droit (cible)
TOLERANCE_LEFT_RIGHT = 0.1;               % ± 10% de tolérance sur la répartition des cycles G et D
...
% === ÉGALISATION DU NOMBRE DE CYCLES ENTRE SURFACES (avec Gauche/Droit) ===
if EQUALIZE_CYCLES_BETWEEN_SURFACES && height(results) > 0
    fprintf('\n⚙️  Égalisation des cycles entre surfaces pour le sujet %s...\n', sujet_id);
    
    rng(RANDOM_SEED_EQUALIZATION);  % seed pour la sélection
    
    nSurf = numel(surfaces);
    leftCounts  = zeros(nSurf,1);
    rightCounts = zeros(nSurf,1);
    totalCounts = zeros(nSurf,1);
    
    idxLeftBySurf  = cell(nSurf,1);
    idxRightBySurf = cell(nSurf,1);
    
    % 1) Compter les cycles Gauche/Droit par surface
    for s = 1:nSurf
        surf = surfaces{s};
        maskSurf = strcmp(results.Surface, surf);
        
        idxL = find(maskSurf & strcmp(results.Cote, 'Gauche'));
        idxR = find(maskSurf & strcmp(results.Cote, 'Droit'));
        
        idxLeftBySurf{s}  = idxL;
        idxRightBySurf{s} = idxR;
        
        leftCounts(s)  = numel(idxL);
        rightCounts(s) = numel(idxR);
        totalCounts(s) = leftCounts(s) + rightCounts(s);
        
        fprintf('  %s : Left=%d, Right=%d, Total=%d\n', ...
            surf, leftCounts(s), rightCounts(s), totalCounts(s));
    end
    
    % Si une surface n'a aucun cycle -> on ne peut pas égaliser
    if any(totalCounts == 0)
        warning('Impossible d''égaliser : au moins une surface n''a aucun cycle. Égalisation sautée.');
    else
        % 2) Déterminer le total max commun et les dispos min Gauche/Droit
        min_total          = min(totalCounts);
        min_left_available  = min(leftCounts);
        min_right_available = min(rightCounts);
        
        % 3) Optimiser la répartition Left/Right (comme l’autre script)
        [n_left_target, n_right_target] = optimize_left_right_split( ...
            min_total, ...
            min_left_available, ...
            min_right_available, ...
            TARGET_LEFT_RIGHT_RATIO, ...
            TOLERANCE_LEFT_RIGHT);
        
        fprintf('  ✂️  Cible par surface : Left=%d, Right=%d (Total=%d)\n', ...
            n_left_target, n_right_target, n_left_target + n_right_target);
        fprintf('  📐 Ratio cible : %.1f%% Left / %.1f%% Right\n', ...
            (n_left_target/(n_left_target+n_right_target))*100, ...
            (n_right_target/(n_left_target+n_right_target))*100);
        
        % 4) Backup des résultats originaux
        results_original = results;   % <--- sauvegarde complète
        
        % 5) Sélection aléatoire dans chaque surface & côté
        keepIdxAll = [];
        for s = 1:nSurf
            surfName = surfaces{s};
            
            % Gauche
            idxL = idxLeftBySurf{s};
            if numel(idxL) > n_left_target
                permL = randperm(numel(idxL), n_left_target);
                keepL = idxL(permL);
                fprintf('  %s Gauche : %d → %d cycles\n', ...
                    surfName, numel(idxL), n_left_target);
            else
                keepL = idxL;
                fprintf('  %s Gauche : %d cycles conservés (<= cible)\n', ...
                    surfName, numel(idxL));
            end
            
            % Droit
            idxR = idxRightBySurf{s};
            if numel(idxR) > n_right_target
                permR = randperm(numel(idxR), n_right_target);
                keepR = idxR(permR);
                fprintf('  %s Droit  : %d → %d cycles\n', ...
                    surfName, numel(idxR), n_right_target);
            else
                keepR = idxR;
                fprintf('  %s Droit  : %d cycles conservés (<= cible)\n', ...
                    surfName, numel(idxR));
            end
            
            keepIdxAll = [keepIdxAll; keepL(:); keepR(:)];
        end
        
        % 6) Appliquer l’égalisation
        keepIdxAll = sort(keepIdxAll);
        results = results(keepIdxAll, :);
        
        fprintf('  ✅ Égalisation terminée : ≈ %d cycles par surface (Left=%d, Right=%d)\n', ...
            n_left_target + n_right_target, n_left_target, n_right_target);
        fprintf('  📊 Total après égalisation : %d cycles\n', height(results));
    end
end
...
if height(results) > 0
    ...
    % Sauvegarde CSV (version égalisée)
    writetable(results, output_file_csv);
    fprintf('✅ CSV généré : %s\n', output_file_csv);
    
    % Sauvegarde MAT
    if ~exist('results_original','var')
        results_original = results; % si pas d'égalisation faite
    end
    
    MoS_data = struct();
    MoS_data.results = results;                    % Résultats après égalisation
    MoS_data.results_original  = results_original; % Le back-up
    MoS_data.sujet = sujet_id;
    MoS_data.surfaces = surfaces;
    MoS_data.essais = essais;
    MoS_data.date_traitement = datetime('now');
    MoS_data.freqVicon = freqVicon;
    MoS_data.equalization_params = struct( ...
        'enabled', EQUALIZE_CYCLES_BETWEEN_SURFACES, ...
        'random_seed', RANDOM_SEED_EQUALIZATION, ...
        'target_left_right_ratio', TARGET_LEFT_RIGHT_RATIO, ...
        'tolerance_left_right', TOLERANCE_LEFT_RIGHT);
    
    save(output_file_mat, 'MoS_data');
    fprintf('✅ MAT généré : %s\n', output_file_mat);
    
    fprintf('📊 Total : %d cycles traités (après égalisation)\n', height(results));
else
...
% tout en respectant le ratio cible et les contraintes
    ...
    % Sinon, ajuster selon les contraintes
    ...
    % Vérifier la tolérance de ratio
    actual_ratio = n_left / (n_left + n_right);
    if abs(actual_ratio - target_ratio) > tolerance
        % Si hors tolérance, on réduit le total en cherchant une solution acceptable
