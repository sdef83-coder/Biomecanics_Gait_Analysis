...
├── 13) Clustering.m
├── 14) Statistic_Clusters.R
├── 14bis) TransitionClusters_Libre.R
├── 15) Kinematics_Analysis.m
├── 16) Statistics_Kinematics_Parameters.m
├── Age_Participant.m
├── ParticipantGroup.m
└── Optional/              # Scripts de visualisation individuels
```
...
**Rôle :** Extraction et visualisation des matrices de cinématique articulaire (cheville, genou, hanche) dans les 3 plans de l'espace, par groupe d'âge et condition.

| | Détail |
|---|---|
| **Entrées** | Fichiers `<ID>_<Condition>.mat` (champ `c.results.MeanLeg.angAtFullCycle`) |
| **Sorties** | `MATRICES<Groupe>.mat` (matrices participants × 100 points), CSV par articulation/plan/condition, figures comparatives (effet âge, effet surface), figures SPM1D |
| **Plans** | Sagittal, Frontal, Transverse |
| **Analyses** | Cycles moyens ± SD, analyse SPM1D (ou SnPM si non-normalité), comparaisons inter-groupes et intra-groupe |
| **Dépendances** | `ParticipantGroup.m`, `spm1dmatlab`, fichiers `.mat` |
| **Précède** | `Statistics_Kinematics_Parameters.m` |

---

### `16) Statistics_Kinematics_Parameters.m`

**Rôle :** Comparaisons statistiques SPM1D des courbes cinématiques entre groupes (Enfants vs Adultes, JeunesEnfants vs Adultes, etc.) et intra-groupe (Plat vs High).

| | Détail |
|---|---|
| **Entrées** | `MATRICES_Enfants.mat`, `MATRICES_Adultes.mat`, `MATRICES_JeunesEnfants.mat` |
| **Sorties** | Figures PNG (comparaisons cinématiques + statistiques SPM), sauvegardées dans `result/Fig/` |
| **Tests** | SPM1D t-test2 (paramétrique) ou SnPM t-test2 (non-paramétrique si test Lilliefors positif) |
| **Dépendances** | `spm1dmatlab`, fichiers `MATRICES*.mat` produits par `Kinematics_Analysis.m` |
...
| Script | Rôle |
|---|---|
| `Optional) Plot_GVI.R` | Boxplot publication du GVI par groupe/surface |
| `Optional) Plot_MoS.R` | Boxplots des MoS (mm et %L0) par groupe/surface |
| `Optional) Plot_SPARC.R` | Boxplot SPARC (Magnitude COM) par groupe/surface |
| `Optional) VAS.R` | Analyse et visualisation de l'échelle VAS (confort perçu) |
| `Optional) Visual-L0_Table.m` | Affiche le tableau des L0 enregistrés |
| `Optional) Visual_FFT_SPARC.m` | Visualisation des spectres FFT pour validation du SPARC |
| `Optional) Visual_GlobalParticipant_MOS.m` | Courbes MoS agrégées par groupe d'âge |
| `Optional) Visual_Individual_MOS.m` | Courbes MoS individuelles (AP et ML, mm et %L0) |
| `Optional) Visual_Kinematics.m` | Vérification cinématique pour un participant |
| `Optional) Visual_Smoothness_Group.m` | Barplots SPARC/LDLJ agrégés par groupe |
...
|---|---|---|
| Jeunes Enfants | 3–6 ans (36–71 mois) | — |
| Enfants | 6–12 ans (72–143 mois) | PAQ-C |
| Adolescents | 12–18 ans (144–216 mois) | PAQ-A |
| Adultes | > 18 ans (> 216 mois) | GPAQ-FR |
